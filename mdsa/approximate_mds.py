...
def run_scmds(num_objects, tile_size, tile_overlap, dim, dist_func, permute_order=True):
    ...
    if num_objects < tile_size:
        raise ValueError("Number of objects cannot be smaller "
                         "than tile size")
    if tile_overlap > tile_size:
        raise ValueError("Tile overlap cannot be bigger than tile size")

    if dim > tile_overlap:
        raise ValueError("Tile overlap must be at least as big as "
